...
/// </summary>
public sealed class AgentDetailViewModel : ScreenBase, IHandle<AgentUpdatedEvent>, IDisposable
{
	...
	public Task InitializeAsync(CancellationToken ct = default) =>
		ConversationsVm.ReloadAsync(ct);
	...
	public async Task Push()
	{
		...
		try
		{
			...
			ApplyServerSnapshot(snapshot, preserveLocalEdits: false);
	...
	private void ApplyServerSnapshot(Agent snapshot, bool preserveLocalEdits = true)
	{
		var variablesAreDirty = !VariablesVm.Variables.SequenceEqual(Agent.Variables);
		var workflowIsDirty = !WorkflowVm.Nodes.SequenceEqual(Agent.Workflow.Nodes);
		Agent = snapshot;
		SystemPromptVm.RefreshFrom(snapshot, preserveLocalEdits);
		FirstMessageVm.RefreshFrom(snapshot, preserveLocalEdits);
		if (!preserveLocalEdits || !variablesAreDirty)
		{
			VariablesVm.RefreshFrom(snapshot);
		}
		if (!preserveLocalEdits || !workflowIsDirty)
		{
